...
{
    pid_obj_t *object = (pid_obj_t *)pvPortMalloc(sizeof(pid_obj_t));  /*!此处申请的动态内存属FreeRTOS里专有的堆管理函数，不要随意使用malloc*/
...
pid_config_t  *init_pid_cfg(float kp, float ki, float kd, float MAXIOut, float MaxOut, float DeadBand, pid_improvement_e Improve)
{
    static pid_config_t *pid_cfg;
    pid_cfg->Kp=kp;
    pid_cfg->Ki=ki;
    pid_cfg->Kd=kd;
    pid_cfg->IntegralLimit=MAXIOut;
    pid_cfg->MaxOut=MaxOut;
    pid_cfg->DeadBand=DeadBand;
    pid_cfg->Improve=Improve;
    return pid_cfg;
}
